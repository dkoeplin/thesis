...
\begin{figure*}
\centering
\includegraphics[height=6cm]{1-intro/figs/system-diag}
...
on the parallel pattern IR (Figure~\ref{fig:system-diag}b.).
...
representation (Figure~\ref{fig:system-diag}c.).
...
The Spatial IR is then optimized and lowered further by the Spatial compiler (Figure~\ref{fig:system-diag}d.).
...
To summarize, the core contributions of this work are as follows:
\vspace{-5pt}
\begin{itemize}
  ...
  first using heurstic search and the second using HyperMapper.
