...
Square brackets (e.g. \texttt{[T]}) represent a template's type parameter. Parameters followed by a `\texttt{+}' denote arguments which can be given one or more times, while a
`\texttt{*}' denotes that an argument is optional.
Subscripts such as $T_D$ denote a tuple type with $D$ elements,
while superscripts such as $T^D$ denotes a D-dimensional memory type.
